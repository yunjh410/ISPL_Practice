...
tf.flags.DEFINE_string('process', 'train', 'which process you want to do: write(write tfrecord file), train(train model), test(test model)')
tf.flags.DEFINE_string('imagedir', './mnist/train', 'directory where images are stored in')
tf.flags.DEFINE_string('datadir', './mnist/train.tfrecord', 'directory where tfrecord files are stored in')
tf.flags.DEFINE_string('val_datadir', './mnist/val.tfrecord', 'directory where validation tfrecord files are stored in')
